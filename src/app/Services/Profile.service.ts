...
export class ProfileservicesService {
  ...
  ChangePass(id:string,newpass:string,oldpass:string):Observable<any>{
    
    return this.http.put<any>(`https://localhost:7223/api/profile/password/${id}/${oldpass}`,newpass)
    .pipe(catchError((err)=>{
      return throwError(()=>err.message ||"server error");
    }));
  }
